...
// --- BİLEŞENLER ---
// SVG Animasyonu: "Çiz ve Doldur" Efekti
const AnimatedLaptopIcon = ({ onAnimationComplete }) => {
  return (
    <motion.div
      ...
      // Çıkış animasyonu: Burası pat diye gitmesini engeller
      exit={{
        opacity: 0,
        scale: 0.5,
        y: -50,
      ...
      >
        ...
        >
          <motion.path
            ...
            transition={{
              duration: 2.0, // Biraz daha hızlandırdım ki kullanıcı sıkılmasın
              ease: "easeInOut",
            }}
    ...
    <>
      ...
      >
        ...
        <motion.div
          ...
          className="flex-[0.75] bg-black-100 p-8 rounded-2xl z-10 border border-white/10 cursor-default shadow-card_bottom"
        >
          <form
            ref={formRef}
        ...
        <motion.div
          ...
          initial="hidden"
          animate={introFinished ? "show" : "hidden"}
          className="xl:flex-1 xl:h-auto md:h-[550px] h-[350px] relative border rounded-2xl border-white/10 cursor-default shadow-card_bottom"
        >
          <div className="w-full h-full rounded-2xl overflow-hidden bg-tertiary/30 border border-white/5 cursor-default">
            <ComposableMap
            ...
            >
              ...
              >
                ...
                <Geographies geography={geoUrl}>
                  {({ geographies }) =>
                    geographies.map((geo) => {
                      if (geo.properties.name === "Turkey") return null;
                      return (
                        <Geography
                          key={geo.rsmKey}
                          geography={geo}
                          ...
                          style={{
                            ...
                            hover: { fill: "#2f314d", outline: "none" },
                ...
                <Geographies geography={geoUrl}>
                  {({ geographies }) =>
                    geographies.map((geo) => {
                      // Türkiye'yi bul - farklı property isimlerini kontrol et
                      const countryName =
                        geo.properties.NAME ||
                        geo.properties.NAME_LONG ||
                        geo.properties.name;
                      const isTurkey =
                        countryName === "Turkey" || countryName === "Türkiye";
                ...
                {/* 3. KATMAN: KONYA DETAYLARI */}
                <Marker coordinates={[32.48, 37.87]}>
                  <circle r={3} fill="white" stroke="#915eff" strokeWidth={2} />
                </Marker>
